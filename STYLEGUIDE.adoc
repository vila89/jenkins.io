...
the `jenkins.io` site. Unlike the link:contributing[CONTRIBUTING] guide, this
STYLEGUIDE covers details about how to use Jenkins-specific terminology, general
comments about phraseology and writing style, as well as all aspects of AsciiDoc
syntax which are relevant to this site.

For details about how to get set up to contribute to the `jenkins.io` site and
broader contribution aspects such as a description of the documentation
structure of `jenkins.io` and how to construct properly functioning content to
the site, see the link:contributing[CONTRIBUTING] guide.
